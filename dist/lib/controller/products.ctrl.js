...
module.exports = {
    ...
    getProductList: function(req, res)
    {
        ...
        connection.acquire(function(err, con){
            ...
            {
                var sql = 'select *, (CASE WHEN unit IS NULL THEN "kg." ELSE unit END) AS unit from products WHERE status = 0 ORDER BY name ASC';
            }
            else{
                var sql = 'select *, (CASE WHEN unit IS NULL THEN "kg." ELSE unit END) AS unit from products WHERE status = 0 AND companyid = '+req.decoded.logedinuser.companyid+' ORDER BY name ASC';
            }
