...
module.exports = {
    ...
    generateInvoice: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                ...
                var sql ="UPDATE `ordermaster` SET `grossamt`= "+req.body[0].totalAmount+", `taxamt`= "+req.body[0].taxamt+",`netamt`= "+req.body[0].netamt+",`pendingpayment` = "+req.body[0].netamt+",`invoicestatus`= 1,`invoicedate`= CURDATE() WHERE `id` = "+req.body[0].orderid+';';
    ...
    getInvoiceListForLumsumPayment : function(req, res)
    {
        ...
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                con.query('select a.id, a.customerid, a.netamt, a.`orderdate`,  a.`invoicedate` from ordermaster a WHERE a.pendingpayment > 0 AND  a.status = 0 AND a.customerid = '+parseInt(req.params.customerid)+' ORDER BY a.id ASC', function(err, result){
                    if(err)
                    {
                        console.log(err)
                        res.send({
