    ...
    saveOrderDetails: function(req, res)
    {
        if (req.decoded.success == true) {  
            if(req.body[0].customerdetails.orderid)
            {
                ...
                var sql = 'UPDATE `ordermaster` SET '+changeCustomer+' `orderdate`= ADDDATE(DATE_FORMAT("'+req.body[0].customerdetails.orderdate+'", "%Y/%m/%d"), INTERVAL 1 DAY) WHERE `id` = '+req.body[0].customerdetails.orderid+';';
    ...
    generateInvoice: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                ...
                req.body.map(function(value){
                    sql = sql+'UPDATE `orderdetails` SET `price`= '+value.rate+',`netprice`= '+value.netprice+' WHERE `id` = '+value.details_id+';'
                });
