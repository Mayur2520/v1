...
module.exports = {
        ...
        if (req.decoded.success == true) {  
            if(req.body[0].customerdetails.orderid)
            {
                ...
                req.body.map(function(value){
                    ...
                    {
                        if(value.orderdetailsid)
                        {
                            sql = sql + 'UPDATE `orderdetails` SET `qty`= '+value.qty+', `unit`= "'+value.unit+'" WHERE `orderid` = '+req.body[0].customerdetails.orderid+' AND `productid` = '+value.id+';';
                        }
                        else
                        {
            ...
            {
            connection.acquire(function(err, con){
                con.query('INSERT INTO `ordermaster`(`customerid`, `orderdate`, `createdby`, `companyid`) VALUES (?,?,?,?)',[req.body[0].customerdetails.customername.id,req.body[0].customerdetails.orderdate,req.decoded.logedinuser.id,req.decoded.logedinuser.companyid], function(err, result){
                    if(err)
                    {
                        console.log('err ----1')
                        console.log(err)
    ...
    getOrderDetails: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                con.query('SELECT *, orderdetails.id as details_id, (SELECT customers.name FROM customers WHERE customers.id = ordermaster.customerid) as cust_name FROM `ordermaster` INNER JOIN orderdetails on orderdetails.orderid = ordermaster.id WHERE ordermaster.id = '+req.params.orderid, function(err, result){
                    if(err)
                    {
                        res.send({status:0, message:'Something went wrong, Please try again.'});
                    }
