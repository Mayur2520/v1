...
module.exports = {
    ...
    getOrderDetails: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                con.query('SELECT *, (SELECT products.type FROM products WHERE products.id = orderdetails.productid) AS type,(SELECT products.marathi_name FROM products WHERE products.id = orderdetails.productid) AS marathi_name, (SELECT products.name FROM products WHERE products.id = orderdetails.productid) AS productname,(CASE WHEN changed_item IS NULL THEN "" ELSE (SELECT products.name FROM products WHERE products.id = orderdetails.changed_item) END) AS changed_productname,orderdetails.id as details_id, (SELECT customers.name FROM customers WHERE customers.id = ordermaster.customerid) as cust_name, (SELECT customers.address FROM customers WHERE customers.id = ordermaster.customerid) as cust_addr, (SELECT customers.mobile FROM customers WHERE customers.id = ordermaster.customerid) as cust_mobile FROM `ordermaster` INNER JOIN orderdetails on orderdetails.orderid = ordermaster.id WHERE ordermaster.id = '+req.params.orderid, function(err, result){
                    if(err)
                    {
                        res.send({status:0, message:'Something went wrong, Please try again.'});
                    }
    ...
    ListPOOrders: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                if(req.decoded.logedinuser.role == 'Superadmin')
            {
                var sql = 'SELECT `id`,status, (SELECT vendors.name FROM vendors WHERE vendors.id = pomaster.customerid) as cust_name,`orderdate`,( SELECT COUNT(*) FROM podetails WHERE podetails.poorderid = pomaster.id) as total_items, (SELECT users.name FROM users WHERE users.id = pomaster.createdby) AS ordered_by FROM `pomaster` WHERE DATE_FORMAT(`orderdate`,"%Y/%m/%d") >= STR_TO_DATE("'+req.body[0].from_orderDate+'","%Y/%m/%d") AND DATE_FORMAT(`orderdate`,"%Y/%m/%d") <= STR_TO_DATE("'+req.body[0].to_orderDate+'","%Y/%m/%d")  ORDER BY id DESC';
            }
            else{
               
                    var sql = 'SELECT `id`, status, (SELECT vendors.name FROM vendors WHERE vendors.id = pomaster.customerid) as cust_name,`orderdate`,`invoicestatus`,cartstatus, `diliverystatus` ,( SELECT COUNT(*) FROM podetails WHERE podetails.poorderid = pomaster.id) as total_items, (SELECT users.name FROM users WHERE users.id = pomaster.createdby) AS ordered_by FROM `pomaster` WHERE status = 0 AND companyid = '+req.decoded.logedinuser.companyid+' AND (DATE_FORMAT(pomaster.orderdate,"%Y/%m/%d") >= STR_TO_DATE("'+req.body[0].from_orderDate+'","%Y/%m/%d") AND DATE_FORMAT(pomaster.orderdate,"%Y/%m/%d") <= STR_TO_DATE("'+req.body[0].to_orderDate+'","%Y/%m/%d")) ORDER BY id DESC';
        ...
        if (req.decoded.success == true) {  
            if(req.body[0].orderid)
            {
                ...
                connection.acquire(function(err, con){
                    ...
                    if(req.body[0].vendorname.id)
                    {
                        var changeVendor = "`customerid`= "+req.body[0].vendorname.id+",";
                    }
                    else
                    {
                        var changeVendor = "";
                ...
                var sql = 'UPDATE `pomaster` SET '+changeVendor+' `orderdate`= "'+moment(req.body[0].orderdate).format("YYYY-MM-DD HH:mm:ss")+'" WHERE `id` = '+req.body[0].orderid+';';
                ...
                req.body.map(function(value){
                    ...
                    
                    {
                         if(value.orderdetailsid || value.details_id)
                        {
                            ...
                            sql = sql + 'UPDATE `podetails` SET `qty`= '+value.qty+','+changed_item+' `unit`= "'+value.unit+'" WHERE `id` = '+detailsId+';';
                        }
                        else
                        {
                            ...
                            sql = sql + 'INSERT INTO `podetails`(`poorderid`, `productid`, `qty`, `dil_qty`, `unit`) VALUES ('+req.body[0].orderid+','+productid+','+value.qty+',"'+value.unit+'");';
                        } 
            ...
            connection.acquire(function(err, con){
                ...
                con.query('SELECT * FROM `pomaster` WHERE `vendorid` = '+req.body[0].vendorname.id+' AND DATE_FORMAT(`orderdate`,"%Y/%m/%d") = "'+req.body[0].orderdate+'"', function(err, result){
                    if(err)
                    {
                        res.send({
                            status: 0,
                ...
                con.query('INSERT INTO `pomaster`(`vendorid`, `orderdate`, `createdby`, `companyid`) VALUES (?,"'+moment(req.body[0].customerdetails.orderdate).format("YYYY-MM-DD HH:mm:ss")+'",?,?)',[req.body[0].vendorname.id,req.decoded.logedinuser.id, req.decoded.logedinuser.companyid], function(err, result){
                    if(err)
                    {
                        console.log(err)
                        res.send({
                    ...
                    else
                    {
                        ...
                        req.body.map(function(value){
                            ...
                            ss = ss+"("+orderid+","+value.id+","+value.qty+","+value.price+",'"+value.unit+"'),";
                        });

                        ss = ss.substr(0, ss.length - 1);
                        
                        con.query("INSERT INTO `podetails`(`poorderid`, `productid`, `qty`, `price`, `unit`) VALUES "+ss, function(err, detailsResult){
                            if(err)
                            {
                                console.log(err)
