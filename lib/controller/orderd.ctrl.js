...
var cryptconf = require('../config/crypt.config');
        ...
        var options = { format: 'Letter', phantomPath:''};
...
module.exports = {
    ...
    getOrderDetails: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                con.query('SELECT *, (SELECT products.type FROM products WHERE products.id = orderdetails.productid) AS type,(SELECT products.marathi_name FROM products WHERE products.id = orderdetails.productid) AS marathi_name, (SELECT products.name FROM products WHERE products.id = orderdetails.productid) AS productname,orderdetails.id as details_id, (SELECT customers.name FROM customers WHERE customers.id = ordermaster.customerid) as cust_name FROM `ordermaster` INNER JOIN orderdetails on orderdetails.orderid = ordermaster.id WHERE ordermaster.id = '+req.params.orderid, function(err, result){
                    if(err)
                    {
                        res.send({status:0, message:'Something went wrong, Please try again.'});
                    }
    ...
    savePaymentDetails : function(req, res)
    {
         if (req.decoded.success == true) {   
            ...
            connection.acquire(function(err, con){
                con.query('INSERT INTO `customerordrpayment`(`orderid`, `customerid`, `paidamt`, `pendingamt`, `paymentdate`, `paidby`, `paymentmode`,`txnno`, `txndate`, `bank`, `createdby`, `companyid`) VALUES (?,?,?,?,"'+req.body.payment_date+'",?,?,?,"'+req.body.txndate+'",?,?,?);UPDATE `ordermaster` SET `pendingpayment` = ? WHERE `id` = ?;',inutparams, function(err, result){
                    if(err)
                    {
                        console.log(err)
                        res.send({
