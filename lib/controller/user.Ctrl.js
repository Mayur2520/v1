    ...
    VerifyCompanyEmail: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                if(req.body.id)
                {
                    var appdenquery = ' AND id != '+req.body.id+'';
                }
                else
                {
                    var appdenquery = ''
                }
                    con.query('SELECT COUNT(*) as emailexist FROM company WHERE email =?'+appdenquery,[req.body.email], function(err,result){
                        if(err)
                        {
                            console.log(err);
    ...
    VerifyCompanyMobile: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                ...
                con.query('SELECT COUNT(*) as mobileexist FROM company WHERE mobile =?'+appdenquery,[req.body.mobile], function(err,result){
                    if(err)
                    {
                        console.log(err);
                        res.send({status:1, message:"Something went wrong"})
                                ...
                                {
                                   {
                                       var mailbody = {
                                        ...
                                        content: 'Dear ' + req.body.CompanyDetails.name + '<br><br><br><h1 style="font-weight:bold;text-align:center;">' + cryptconf.decrypt(password) + '</h1> <br> <p>enter this as a  password for the app.<br><br><br><br> <div style="float:left;text-align:left;">Thanks, <br> Admin <br> (L.N. software Pvt. Ltd.)</div></p>' // plain text body
                                       }
                                       sendMail(mailbody)
                                   }
